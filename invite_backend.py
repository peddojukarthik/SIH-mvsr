...
def case_members(case_id: str, authorization: str | None = Header(default=None)):
    ...
    try:
        r = supabase.table("case_membership").select(
            "user_id,permission_level,allowed_document_types,granted_by,delegated_by,users(employee_id,employee_registry!fk_users_employee(full_name,departments(name)))"
